

examenClinique: {
        observations: {
          taille: String,
          poids: String,
          imc: String,
          tension: String,
          glycemie: String,
          saturationO2: String,
          bilioGTH: String,
          bilioCardio: String,
          commentairesCardio: String,
          bilioORL: String,
          commentairesSpirometrie: String,
          audiogramme: String,
          douleurs: String,
          commentairesDouleurs: String



test







## SECTION CONCLUSION ##


# Conseils de prévention
Conseils donnés :
Troubles liés au travail : (Liste à synchroniser) : TMS, A,B,C,D,E


# Limitation de travail
Limitation : O/N
Durée de limitation : case à cocher  
    * Déinitive
    * Temporaire 
        (Si temporaire) --> * Durée (j) : 
Commentaire Limitation :

# Action 1 : Orientations
Orientation : Selon liste orientation (déjà synchronisé dans liste manager)
Commentaire : 

# Action 2 : Etude de poste  
Etude à faire : O/N
Commentaire :

# Action 3 : Manager
Entretien manager : : O/N
Manager : Selon liste 
Commentaire pour entretien :
Date pour rappel : 

# Action 4 : Entretien 
Entretien infirmier à prévoir : O/N
Date pour rappel : 


# Action 5 : Medecin
Echange médecin Travail : O/N
Commentaire : 

Action 6 : Visite médicale
A Plannifier : O/N 
Date pour rappel : 
Commentaire pour prochaine visite : 










# Formulaire médical

# Biométrie
- Taille (cm) : champ à remplir
- Poids (kg) : champ à remplir
- Tensions : champ à remplir
- Pouls : champ à remplir
- Température : champ à remplir
- Glycémie : champ à remplir
- SAT : champ à remplir
- IMC : champ à remplir

# Appareil

## Yeux / annexes
- Bilan OPH : choix Oui/Non
- Commentaires ORL : zone de texte
- Commentaires OPH : zone de texte

## Cardio-pulmonaire
- Bilan Cardio : choix Oui/Non
- Commentaires Cardio : zone de texte

## Appareil digestif
- Commentaires appareil digestif : zone de texte

## Uro-génital
- Suivi gynéco : choix Oui/Non
- Dernier frottis, mamographie, hydratation : zone de texte

## Ostéo-Articulaire
- Plainte évoquée : choix Oui/Non
- Commentaires douleurs rhumato : zone de texte

## Neuro-psy
- Sommeil bon ? : choix Oui/Non
- Commentaires neuro-psy : zone de texte

## Endocrino et métabolisme
- Dernier bilan ? : zone de texte

# Antécédents

## Antécédents médicaux
- Question Oui/Non
- Quoi ? : zone de texte
- Commentaires médicaux : zone de texte

## Antécédents chirurgicaux
- Question Oui/Non
- Quoi ? : zone de texte
- Commentaires chirurgicaux : zone de texte

# Traitements

## Traitement(s)
- Question Oui/Non
- Quoi ? : zone de texte
- Commentaires traitements : zone de texte

## Vaccination
- Vaccination à jour ? : choix Oui/Non
- Souhaitez-vous participer à la campagne de vaccination COVID-19 chez Medtronic ? : choix Oui/Non
- Commentaires vaccins : zone de texte

# Section Conclusion

## Conseils de prévention
- Conseils donnés
- Troubles liés au travail : TMS, A, B, C, D, E

## Limitation de travail
- Limitation : Oui/Non
- Durée de limitation : 
  * Définitive
  * Temporaire (Si temporaire → Durée en jours)
- Commentaire Limitation

# Actions à suivre

## Action 1 : Orientations
- Orientation selon liste prédéfinie
- Commentaire

## Action 2 : Étude de poste
- Étude à faire : Oui/Non
- Commentaire

## Action 3 : Manager
- Entretien manager : Oui/Non
- Manager selon liste
- Commentaire pour entretien
- Date pour rappel

## Action 4 : Entretien
- Entretien infirmier à prévoir : Oui/Non
- Date pour rappel

## Action 5 : Médecin
- Échange médecin Travail : Oui/Non
- Commentaire

## Action 6 : Visite médicale
- À Planifier : Oui/Non
- Date pour rappel
- Commentaire pour prochaine visite